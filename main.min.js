...
$jscomp.polyfillIsolated = function(a, b, c, d) {
    ...
    null != b && (a ? $jscomp.defineProperty($jscomp.polyfills, e, {
        ...
        writable: !0,
        value: b
    }) : b !== c && (void 0 === $jscomp.propertyToPolyfillSymbol[e] && (c = 1E9 * Math.random() >>> 0,
    $jscomp.propertyToPolyfillSymbol[e] = $jscomp.IS_SYMBOL_NATIVE ? $jscomp.global.Symbol(e) : $jscomp.POLYFILL_PREFIX + c + "$" + e),
...
function onLoginSubmitted() {
    ...
    1 > document.login.username.value.length ? (a.innerText = "يجب ادخال قيمة صحيحة!",
    showErrorPopup(),
    animating = !1) : (a.innerText = "",
...
function showLoginPage() {
    loggedIn ? showStatusPage() : ($login.style.display = "block",
...
function initHotspotEvents(a, b) {
    var c = b.querySelectorAll(".app-logout"), d;
    a.addEventListener("click", function(f) {
        animating || (animating = !0,
        "status" === b.id ? onLoginSubmitted() : (this.classList.add("processing"),
        showParentElement(this, b)))
    });
    for (d in c) {
        var e = c[d];
...
function toArabicTime(a) {
    ...
    if ("" === a)
        return "";
    0 <= a.indexOf("d") && (b = a.split("d"),
    c = "" !== b[0] ? b[0] + " يوم " : "",
    b = b[1]);
    0 <= a.indexOf("h") && (b = b.split("h"),
    d = "" !== b[0] ? b[0] + " ساعة " : "",
    b = b[1]);
    0 <= a.indexOf("m") && (b = b.split("m"),
    e = "" !== b[0] ? b[0] + " دقيقة " : "",
    b = b[1]);
    0 <= a.indexOf("s") && (a = b.split("s"),
    f = "" !== a[0] ? a[0] + " ثانية " : "");
    return c + d + e + f
}
...
function onLoggedIn(a) {
    loggedIn = !0;
    remember();
    if (null == getCookieOrNull("captive_helper") && getDeviceType().includes("Captive"))
        return setCookie("captive_helper", "1", .5),
        location.reload();
    ...
    if (a.logged_in)
        for (var b in a) {
            ...
            if ("string" == typeof c && "logged_in" !== b) {
                ...
                null != d && ("" === c ? (c = document.querySelector("[".concat(b, "]")),
                null != c && (c.style.display = "none")) : "username" === b ? d.innerText = hideHalfCard(c) : b.includes("bytes") ? d.innerText = toArabicBytes(c) : b.includes("time") ? d.innerText = toArabicTime(c) : d.innerText = c)
            }
        }
    else
        loggedIn = !1
        ...
        b.onreadystatechange = function() {
            if (4 === this.readyState) {
                ...
                try {
                    ...
                    "/status?var=callBack" === a && (loggedIn = 0,
                    hideStatusPage(),
                    showLoginPage(),
                    cookieLogin())
                }
...
function userLogin() {
    var a = document.login.querySelector("input[username-field]").value, b, c = null != (b = document.login.querySelector("input[password-field]")) ? b : "";
    "" !== c && (c = c.value);
...
function startStatusQuery() {
    if (!intervalStatus && loggedIn) {
        ...
        var a = setInterval(function() {
            loggedIn ? getRequest("/status?var=callBack") : (intervalStatus = !1,
            clearInterval(a))
        }, 1E3)
    }
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    var a = document.querySelectorAll("button[parent-id]"), b;
    for (b in a) {
        var c = a[b];
        if ("object" == typeof c) {
    ...
    setTimeout(function() {
        return getRequest("/login?var=callBack")
    }, 50)
}, !1);